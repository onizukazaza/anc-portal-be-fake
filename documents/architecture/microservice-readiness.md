...
ทุก module รับ `Deps` struct เดียว → **ขอบเขตชัด, ตัดออกได้ง่าย**

---

## 3. Readiness Score — พร้อมแค่ไหน

### คะแนนรวม: **8.0 / 10 — พร้อมสำหรับ Phase 1 Extraction**

| ด้าน | คะแนน | สถานะ |
|------|-------|-------|
| Module Boundary Separation | 9/10 | ✅ Hexagonal pattern ทุก module |
| Dependency Injection | 9/10 | ✅ `module.Deps` central struct |
| Event-Driven (Kafka) | 9/10 | ✅ Producer/Consumer/Router/DLQ/Tracing |
| Database Multi-Tenancy | 8/10 | ✅ Provider pattern + External DBs |
...
### 4.3 Database Provider — Multi-DB

```go
// interface รองรับหลาย DB
type Provider interface {
    Main() *pgxpool.Pool                         // Primary DB
    External(name string) (*pgxpool.Pool, error)  // External/Legacy DBs
...
- แต่ละ module เรียก DB ผ่าน interface → สลับ implementation ได้
- External DBs รองรับ partner databases หลายตัว
...
### 4.6 แยก Binary แล้ว — 6 Entry Points
...
| Database Provider Interface | แยก DB ต่อ service ได้ |
